...
extension Compress on IVideoCompress {
  ...
  Future<File> getFileThumbnail(
    ...
    final filePath = await (_invoke<String>('getFileThumbnail', {
      'path': path,
      'quality': quality,
      'position': position,  // 直接使用传入的 position，0 表示第一帧
    }));
  ...
  Future<MediaInfo> getMediaInfo(String path) async {
    ...
    if (jsonStr != null) {
      try {
        ...
        debugPrint("IVideoCompress.getMediaInfo json decode error: $e");
      }
    }
  ...
  // 获取压缩临时文件存储路径
  Future<String> getCompressDir() async {
    final dir = await _invoke<String>('getCompressDir');
    return dir ?? "";
  }

  // 获取压缩临时文件存储总大小
  Future<double> getCompressSize() async {
    double totalSize = 0;
    String cacheDir = await getCompressDir();
    final cacheDirectory = Directory(cacheDir);
    if (!await cacheDirectory.exists()) {
      return totalSize;
    }

    await for (var fileEntity in cacheDirectory.list(recursive: true)) {
      if (fileEntity is File) {
        totalSize += await fileEntity.length();
      }
    }

    return totalSize; // 返回总大小（字节）
  }
